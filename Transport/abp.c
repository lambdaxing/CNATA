...
/* 保持可靠数据传输的状态变量 */
int seqnumA;              // A 的序号：0，1 表示当前发送的分组序号。
int statusA;              // A 的状态：0 表示处于等待上层调用状态，1 表示处于等待 ACK 状态。
struct pkt sndpktA;       // A 的当前分组 package，停等协议一次发送一个分组
struct pkt sndpktB;       // B 的当前分组 package，停等协议一次接受一个分组
int seqnumB;              // B 的当前序号，表示将接收的分组序号，与 A 的序号对应。
...
void A_output(message)
  struct msg message;
{
  if(statusA == 1)                                  // 处于等待 ACK 状态
    return;
  sndpktA.seqnum = seqnumA;                         // 打包分组...
  sndpktA.acknum = 0;                               // 0 表示非 ACK/NCK 分组
  for(int i = 0; i < 20; i++)
    sndpktA.payload[i] = message.data[i];
  sndpktA.checksum = checksum(sndpktA);
  tolayer3(0, sndpktA);
  starttimer(0, 12.0);
  statusA = 1;                                      // 进入 等待ACK 状态
  seqnumA = (seqnumA+1)%2;                          // 改变序号
}
...
void A_input(packet)
  struct pkt packet;
{
  if(statusA == 0)                                                    // 处于等待上层调用状态
    return;
  if(packet.checksum != checksum(packet) || packet.acknum == -1)      // A/NCK分组损坏 or NAK分组到达
    return;
  if(packet.seqnum != (seqnumA+1)%2)                                  // 失序 ACK/NAK 分组到达
    return;
  if(packet.checksum == checksum(packet) && packet.acknum == 1){      // 当前等待的 ACK 分组到达
    statusA = 0;                                                      // 进入 等待上层调用 状态
    stoptimer(0);                                                     // 停止计时器
  }
}
...
{
  tolayer3(0, sndpktA);                                               // 重发当前分组
  starttimer(0, 12.0);                                                // 开始计时  
}  
...
void A_init()
{
  seqnumA = statusA = 0;                                              // 初始设置状态和序号都为 0
}
...
void B_input(packet)
  struct pkt packet;
{
  if(packet.checksum == checksum(packet)) {                           // 接收分组无损坏
    if(packet.seqnum == seqnumB){                                     // and 接收分组序号正确
      tolayer5(1, packet.payload);                                    // 上递分组数据
      seqnumB = (seqnumB+1)%2;                                        // 更新序号
    }
    sndpktB.acknum = 1;                                               // 回送 ACK
  }else {                                                             // 接收分组损坏
    sndpktB.acknum = -1;                                              // 回送 NAK
  }
  sndpktB.seqnum = packet.seqnum;                                     // 打包 ACK/NAK 分组
  sndpktB.checksum = checksum(sndpktB);
  tolayer3(1, sndpktB);                                               // 发送
}
